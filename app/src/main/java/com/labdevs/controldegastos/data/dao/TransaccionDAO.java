...
public interface TransaccionDAO {
    ...
    @Query("SELECT sum(t.monto) AS 'monto', c.nombre FROM transacciones AS t JOIN categorias AS c ON t.id_categoria = c.id WHERE t.tipo_transaccion LIKE :tipoTrans AND date(t.fecha_hora) BETWEEN :fechaDesde AND :fechaHasta GROUP BY c.nombre")
    List<ItemInforme> listarPorPeriodo(String tipoTrans, String fechaDesde, String fechaHasta);

    @Query("SELECT sum(t.monto) AS 'monto', c.nombre FROM transacciones AS t JOIN categorias AS c ON t.id_categoria = c.id WHERE t.tipo_transaccion LIKE :tipoTrans AND strftime('%m',t.fecha_hora) = :mes GROUP BY c.nombre")
    List<ItemInforme> listarPorMes(String tipoTrans, String mes);

    @Query("SELECT sum(t.monto) AS 'monto', c.nombre FROM transacciones AS t JOIN categorias AS c ON t.id_categoria = c.id WHERE t.tipo_transaccion LIKE :tipoTrans AND strftime('%Y',t.fecha_hora) = :year GROUP BY c.nombre")
    List<ItemInforme> listarPorAnio(String tipoTrans, String year);
